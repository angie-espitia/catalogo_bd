...
def mysql_db(request):
	db = MySQLdb.connect(user='root', db='mysql', passwd='lucasmateo', host='localhost')
	cursor = db.cursor()
...
def postgres_db(request):
	db2 = psycopg2.connect(user='postgres', dbname='postgres', password='lucasmateo', host='localhost')
	cursor = db2.cursor()
...
def listar_tablas_campos_mysql(request):
	...
	db = MySQLdb.connect(user='root', db=dbs, passwd='lucasmateo', host='localhost')
	cursor = db.cursor()
	value = request.POST.get('value')
...
def listar_tablas_postgres(request):
	...
	value = request.POST.get('value')
	db = psycopg2.connect(user='postgres', dbname=value, password='lucasmateo', host='localhost')
	cursor = db.cursor()
...
def listar_tablas_postgres_catalogo(request):
	...
	db = psycopg2.connect(user='postgres', dbname='postgres', password='lucasmateo', host='localhost')
	cursor = db.cursor()
...
def listar_tablas_campos_postgres(request):
	dbs = request.POST.get('db')
	db = psycopg2.connect(user='postgres', dbname=dbs, password='lucasmateo', host='localhost')
	cursor = db.cursor()
	value = request.POST.get('value')
	query = "select * from %s" %value
	cursor.execute(query)
